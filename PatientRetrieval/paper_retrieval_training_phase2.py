...
class complete_model(pl.LightningModule):
    def __init__(self,
        ...
        debug_df = get_data_df(root, "/nas-ctm01/homes/fpcampos/dev/diffusion/medfusion/data/mimic-cxr-2.0.0-split.csv", split="test")
        self.test_set_val = eval_Dataset(root, debug_df, debug_df) # TODO
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=self.hparams.learning_rate,
                                                        ...
                                                        total_steps=(int(1627703 / 16) + 1) * self.hparams.expected_num_epochs + 1,
                                                        epochs=None,
...
class miningDataset(data.Dataset):
    def __init__(self, root, patient_data_frame, train, bucket_size=8):
        ...
        print("Len of train", len(self.db.train_set))
        print("Len of val", len(self.db.val_set))
        ...
        if self.train:
            transform_list = [
                tv.transforms.Resize([512, 512], interpolation=2),
...
class MiningDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage == 'test':
            self.test_set = eval_Dataset(self.root_images, get_data_df(self.root, self.split_path, split="test"), get_data_df(self.root, self.split_path, split="validate")) # TODO
    ...
    def val_dataloader(self):
        self.val_set = eval_Dataset(self.root_images, get_data_df(self.root, self.split_path, split="validate"), get_data_df(self.root, self.split_path, split="validate")) # TODO: Fix this
