...
class SiameseDataset(data.Dataset):
    def __init__(self, phase='training', data_handling='randomized', n_channels=3, n_samples=792294, transform=None,
        ...
        if self.phase == 'training':
            ...
            filenames = np.loadtxt('./mimic_train_val_list.txt', dtype=str)[5138:]
            self.image_pairs = np.loadtxt('./image_pairs/pairs_mimic_training.txt', dtype=str)
        elif self.phase == 'validation':
            filenames = np.loadtxt('./mimic_train_val_list.txt', dtype=str)[:5138]
            self.image_pairs = np.loadtxt('./image_pairs/pairs_mimic_validation.txt', dtype=str)
        elif self.phase == 'testing':
            filenames = np.loadtxt('./mimic_test_list.txt', dtype=str)
            self.image_pairs = np.loadtxt('./image_pairs/pairs_mimic_testing.txt', dtype=str)
        else:
